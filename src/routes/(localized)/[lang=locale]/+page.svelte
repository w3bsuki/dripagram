<script lang="ts">
	...
	let storiesProducts = $state(data.storiesProducts || []);
	...
	$effect(() => {
		...
		// Keep stories products static - only update if we have fresh data (initial load)
		if (data.storiesProducts && data.storiesProducts.length > 0) {
			storiesProducts = data.storiesProducts;
		}
	...
	async function handleProductLike(productId: string) {
		try {
			...
			if (!response.ok) {
				...
				const product = products.find((p: FeedProduct) => p.id === productId);
				if (product) {
