...
// Reuse the same feed loading functions from +page.server.ts
async function loadForYouFeed(
	...
	const products = (rawProducts?.slice(0, pageSize) || []).map((product: any) => 
...
async function loadTrendingFeed(
	...
	const productsWithScores = (engagementData || [])
		.map((item: any) => {
			...
			return {
				...product,
				engagement_score: isPromoted ? engagementScore + 100 : engagementScore,
	...
	const products = productsWithScores
		.slice(0, pageSize)
...
function transformProduct(product: any, userLikes: string[]) {
