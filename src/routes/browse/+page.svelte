<script lang="ts">
  ...
  const trendingSearches = [
    'Vintage denim', 'Nike sneakers', 'Summer dresses', 'Designer bags', 
    'Streetwear', 'Y2K fashion', 'Cottagecore', 'Minimalist style'
  ];
  ...
  const activeFiltersCount = $derived([selectedCategory, selectedCondition, selectedSubcategory].filter(Boolean).length);
  const currentSubcategories = $derived(selectedCategory ? subcategories[selectedCategory as keyof typeof subcategories] || [] : []);
</script>
